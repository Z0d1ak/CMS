...
function Instuments(props:{ delete:()=>void, edit:()=>void}) {
    return(
        <Row>
            <Col span={1}></Col>
            <Col span={6}>
                <Button className={"buttonOption"} onClick={props.delete}>delete</Button>
            </Col>
            <Col span={10}></Col>
            <Col span={6}>
                <Button className={"buttonOption"} onClick={props.edit} >edit</Button>
            </Col>
            <Col span={1}></Col>
        </Row>
    );
...
function Cell(props:{x:number,y:number,width:number,content:JSX.Element,
    insertComponent:(x:number,y:number,width:number,component:JSX.Element)=>void,
    deleteComponent:(x:number,y:number,width:number)=>void}
    ) {
    ...
    const [{ isOver }, drop] = useDrop(
        () => ({
          ...
          drop: (item:any) => {
              ...
              props.insertComponent(props.x,props.y,3,<>{ComponentsBlock[item.pos]}<Instuments delete={()=>{props.deleteComponent(props.x,props.y,3); console.log("del")}}  edit={()=>{console.log("1")}}/></>)
          },
          collect: (monitor) => ({
    ...
    return(
        <Col span={props.width}>
            <div ref={drop} className="redactorCell">{props.content}</div>
        </Col>
    ...
    export class ArticleBlock extends React.Component<{
        ...
        addRow=()=>{
            ...
            for (var j = 0; j < this.props.cels; j++) {
                gridRow.splice(j,0,<Cell x={j} y={this.props.rows+1} width={1} content={<></>} 
                insertComponent={this.insertComponent} deleteComponent={this.deleteComponent}/>);
            }
            buf.splice(buf.length,0,gridRow);
            this.setState({grid:buf})
        }

        deleteComponent=(x:number, y:number,width:number)=>{
            console.log("delete");
            let buf:JSX.Element[][]=this.state.grid;
            buf[y].splice(x,1);
        ...
        constructor(props:{rows:number
            ...
            for (var i = 0; i < this.props.rows; i++) {
                let gridRow:JSX.Element[] =[]
                for (var j = 0; j < this.props.cels; j++) {
                   gridRow.splice(j,0,<Cell x={j} y={i} width={1} content={<></>} insertComponent={this.insertComponent} deleteComponent={this.deleteComponent}/>);
                }
